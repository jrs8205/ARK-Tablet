...
public class WeatherData {
    ...
    public static class Current {
        ...
        public double precip1h = Double.NaN; // most recent r_1h observation (mm/h)
    ...
    /** Timestamp of the last successful forecast fetch. 0 = not fetched yet.
     *  Used to decide whether the forecast refetch can be skipped on the next
     *  observation cycle. */
    public long forecastFetchedAt = 0;

    /** Rounding: |x| < 0.5 -> 0.0 so that -0 C never shows up. */
    ...
    public static double computeFeelsLike(double tC, double windMs, double humidity) {
        ...
        if (tC >= 26.7 && !Double.isNaN(humidity) && humidity > 40.0) {
            // Heat index (Steadman, short form)
