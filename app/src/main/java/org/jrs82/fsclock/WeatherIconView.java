...
public class WeatherIconView extends View {
    ...
    /** WeatherCondition -> internal drawing symbol. */
    ...
    public static int mapCondition(WeatherCondition c) {
        ...
        // Primarily rawSmartSymbol: distinguish partly cloudy rain/sleet/snow
        // (21/31-33, 41-43, 51-53) vs. mostly cloudy / overcast rain etc. (34-39/44-49/54-59).
        if (c.rawSmartSymbol != null && c.rawSmartSymbol > 0) {
            int ss = c.rawSmartSymbol;
            int base = ss % 100;            // without the night offset
            boolean night = ss >= 100 || c.isNight;
            // Showers (partly cloudy + rain) and partly cloudy rain
            ...
            // 37-39 -> plain rain (falls through to the type stage).
            // Sleet: showers and partly cloudy sleet
            if (base >= 41 && base <= 43) {
            ...
            // 47-49 -> plain sleet (fallthrough).
            // Snow: showers and partly cloudy snowfall
            if (base >= 51 && base <= 53) {
            ...
            // 57-59 -> plain snowfall (fallthrough).
            // 61/64/67 hail showers and 71/74/77 thundershowers -> fall through to the type stage.
        }
        switch (c.type) {
    ...
    /** Small sun/moon at the top right, largely hidden behind the cloud.
     *  Used in the "mostly cloudy + rain/sleet/snow" icons. */
    private void drawTinySunOrMoon(Canvas c, float cx, float cy, float s, boolean night) {
        ...
        if (night) {
            ...
            // Only the top and top-right rays, since the cloud covers the rest
    ...
    private void drawMostlyRain(Canvas c, float cx, float cy, float s, boolean night) {
        ...
        // Larger cloud than in the partly version, but smaller than plain rain
    ...
    /** Draw a small sun (or moon) at the top right. Used in the partly cloudy
     *  precipitation icons. The cloud is drawn on top afterwards so that
     *  the sun/moon remains partially visible. */
    ...
    private void drawCloud(Canvas c, float cx, float cy, float s, int color) {
        ...
        paint.setStyle(Paint.Style.FILL);
        // cloud - 3 circles + a rectangle base
        float r1 = s * 0.18f;
        float r2 = s * 0.22f;
        float r3 = s * 0.16f;
        float by = cy + s * 0.05f;
    ...
    private void drawSunCloud(Canvas c, float cx, float cy, float s) {
        // sun behind it, up and to the left
