...
/** Internal weather model. The UI and WeatherIconView use this, not FMI's raw numbers. */
public class WeatherCondition {
    ...
    // ============================================================
    // SmartSymbol mapping (primary)
    // Source: https://en.ilmatieteenlaitos.fi/weather-symbols
    //   1 clear, 2 mostly clear, 4 partly cloudy, 6 mostly cloudy, 7 overcast
    //   9 fog
    //   11 drizzle, 14 freezing drizzle, 17 freezing rain
    //   21/24/27 isolated/scattered/showers (rain showers)
    //   31-39 rain (three cloud backgrounds × three intensities)
    //   41-46 sleet showers, 47-49 sleet
    //   51-56 snow showers, 57-59 snowfall
    //   61/64/67 hail showers (no HAIL type → SLEET shower fallback)
    ...
    public static WeatherCondition fromSmartSymbol(int smartSymbol) {
        ...
        switch (n) {
            ...
            // Drizzle and freezing precipitation
            ...
            // Rain showers 21/24/27 = isolated / scattered / showers
            ...
            // Rain 31-39: partly cloudy+rain (31-33), mostly cloudy+rain (34-36),
            // overcast rain (37-39). Intensity 1/2/3 = LIGHT/MODERATE/HEAVY.
            ...
            // Sleet showers 41-46 (isolated/scattered, light/moderate/heavy)
            ...
            // Hail showers 61/64/67 — no HAIL type; a SLEET shower is used as the fallback
            // TODO: add a HAIL type if hail should get its own icon
    ...
    // ============================================================
    // WeatherSymbol3 mapping (fallback)
    // ============================================================
    ...
    // ============================================================
    // wawa observation code (a refinement; does not force the current icon to change)
    // ============================================================
    public static WeatherCondition fromWawa(int wawa, WeatherCondition fallback) {
        ...
        if (wawa >= 20 && wawa <= 25) {
            // phenomenon occurred during the past hour — do not force the present into rain
            if (fallback != null) {
                c.type = fallback.type;
                c.intensity = fallback.intensity;
                c.isShower = fallback.isShower;
        ...
        if (wawa >= 40 && wawa <= 42) {
            // WMO 4680: 40 = precipitation (unspecified), 41 = light/moderate, 42 = HEAVY.
            c.type = Type.RAIN;
        ...
        if (wawa >= 50 && wawa <= 56) {
            // Drizzle / freezing drizzle. WMO 4680: 50/51/54 = light, 52/55 = moderate,
            // 53/56 = HEAVY. Shown as rain according to the drizzle intensity.
            c.type = Type.RAIN;
        ...
        if (wawa >= 80 && wawa <= 84) {
            // WMO 4680: 80 = shower (intensity unspecified), 81 = light,
            // 82 = moderate, 83 = heavy, 84 = violent.
        ...
        if (wawa == 89) {
            // TODO: 89 = a hail shower may occur with either rain or thunder
    ...
    public static WeatherCondition fromWmoCode(int code, boolean night) {
        ...
        switch (code) {
            ...
            // 1 = "mainly clear" — same class as FMI SmartSymbol 2 (mostly
            // clear) → PARTLY_CLOUDY, so the FMI/OM comparison columns show
            // the same icon in the same weather.
            ...
            // Freezing drizzle/rain is not sleet — shown as rain like the FMI path
            // (SmartSymbol 14/17 → RAIN), so both sources show the same icon.
