...
public class SettingsFragment extends PreferenceFragmentCompat {
    ...
    /** True while home_place is set programmatically via the dialog — prevents
     *  the OnPreferenceChangeListener from opening the dialog a second time. */
    private boolean settingHomePlaceProgrammatically = false;

    /** Ruuvi scan dialog state (if the dialog is open). */
    private AlertDialog ruuviScanDialog;
    /** Order of the MACs shown in the dialog (same as displayed in the list). */
    private final List<String> ruuviDialogMacs = new ArrayList<>();
    /** Slot the selected sensor is assigned to, or null if asked separately in the dialog. */
    private String ruuviPendingSlot;
    /** Listener through which RuuviRepository updates the open dialog. */
    private RuuviRepository.Listener ruuviDialogListener;
    /** Run when the BLUETOOTH_SCAN permission is granted (if a dialog open was attempted). */
    ...
    public void onCreatePreferences(Bundle savedInstanceState, String rootKey) {
        ...
        // EditTextPreference: the place summary shows the text by default
        ...
        // Auto-time warning: shown only if automatic time is off
    ...
    /** Changing the home place requires confirmation, because the new
     *  DB channel starts empty. The listener returns false and shows a dialog;
     *  OK sets the value manually behind a guard flag so the dialog does not reopen. */
    ...
    private void showHomePlaceChangeDialog(final EditTextPreference pref, final String newPlace) {
        new AlertDialog.Builder(requireContext())
                ...
                .setPositiveButton(R.string.home_place_change_ok, (d, w) -> {
                    ...
                    try {
                        // setText writes to SharedPreferences under the "home_place" key
                        // and triggers FsClockApp's channel switch. No separate
                        // SettingsManager.setHomePlace call is needed.
                        SettingsManager.get().clearHomeCoordinates();
                        // Manually set home place: GPS tracking off so it does not
                        // overwrite the choice right at the next startup.
    ...
    /** Editing the sensor's display name. The EditTextPreference writes directly
     *  to the same SharedPreferences key that SettingsManager.getRuuviName reads.
     *  The summary shows the current name, or the default name if the field is empty. */
    ...
    /** Assigns the MAC to a slot. If the MAC was previously in another slot, it is removed from there. */
    private void assignMacToSlot(String mac, String slot) {
        SettingsManager sm = SettingsManager.get();
        // remove the MAC from a possible old slot
        String existing = sm.slotForMac(mac);
        if (existing != null && !existing.equals(slot)) {
            sm.setRuuviMac(existing, null);
        }
        // remove a possible previous MAC from the target slot (one sensor per slot)
    ...
    /** Checks whether the Bluetooth radio is on. Try/catch because adapter.isEnabled()
     *  may throw a SecurityException on old API levels if the permission is missing. */
    ...
    /** Ensures the permission required for BLE scanning: API 31+ BLUETOOTH_SCAN, API ≤30
     *  ACCESS_FINE_LOCATION. Returns true if already granted, false if the user is
     *  being asked (in which case onPermissionResult runs the requested action). */
